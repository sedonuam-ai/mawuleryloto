...
function renderHome() {
  ...
  showScreen('home', 'LOTO-SAME-GLOBAL SERVICE');
}
...
// Réinitialise complètement l'historique (avec double confirmation pour
// éviter une suppression accidentelle)
document.getElementById('reset-all-btn').addEventListener('click', () => {
  const totalDraws = getAllDraws().length;
  if (totalDraws === 0) {
    alert('Aucun résultat enregistré pour le moment.');
    return;
  }
  const confirmed = confirm(
    `Es-tu sûr de vouloir effacer les ${totalDraws} résultat(s) enregistré(s) ?\nCette action est définitive.`
  );
  if (!confirmed) return;

  const doubleCheck = confirm('Dernière confirmation : tout effacer ?');
...
document.getElementById('generate-btn').addEventListener('click', () => {
  ...
  // Dernier tirage officiel connu pour ce périmètre, utilisé pour repérer
  // automatiquement les grilles gagnantes parmi celles générées
  const lastDraw = draws.length
    ? draws.slice().sort((a, b) => b.date.localeCompare(a.date))[0]
    : null;
  ...
  for (let i = 0; i < count; i++) {
    ...
    const matches = lastDraw ? grid.filter(n => lastDraw.numbers.includes(n)) : [];
    const paires = getPaires(matches.length);
    const isWinning = lastDraw && matches.length >= 1;

    const item = document.createElement('div');
    item.className = 'history-item';
    item.innerHTML = `
      <div class="history-date" style="display:flex; align-items:center; gap:6px;">
        ${isWinning ? '<span class="blinking-dot"></span>' : ''}
        Grille n°${i + 1}
        ${isWinning ? `<span style="color:#e53935; font-weight:bold;">— ${paires} paire(s) gagnée(s)</span>` : ''}
      </div>
      <div>${grid.map(n => `<span class="num-circle" style="${lastDraw && lastDraw.numbers.includes(n) ? 'background:#FFD700; color:#1B5E20;' : ''}">${n}</span>`).join('')}</div>
    `;
  ...
  if (!lastDraw) {
    ...
    note.textContent = 'Aucun tirage officiel enregistré pour ce périmètre : impossible de détecter les grilles gagnantes pour l\'instant.';
