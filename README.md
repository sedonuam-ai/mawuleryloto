# LOTO-SAME-GLOBAL SERVICE - Application installable (sans Flutter, sans compilation)
...
1. Décompresse ce zip : tu obtiens un dossier `loto_togo_web/` avec ces fichiers,
   **tous à la racine, sans sous-dossier** (volontairement, pour éviter les
   problèmes d'upload sur GitHub) :
   ```
   index.html
   app.js
   manifest.json
...
4. Glisse-dépose TOUS ces fichiers (pas de dossier cette fois, juste les fichiers
   directement).
5. En bas de page, clique sur **"Commit changes"**.

⚠️ Important : ces fichiers doivent être directement à la racine du dépôt (au
même niveau que ton README.md), pas dans un sous-dossier — sinon le lien
GitHub Pages ne les trouvera pas.

⚠️ Si tu avais déjà un ancien dossier `icons/` sur ton dépôt (d'une version
précédente), tu peux le supprimer : clique sur le dossier `icons` sur GitHub,
puis sur chaque fichier dedans, puis l'icône 🗑️ pour le supprimer — ou laisse-le,
il ne sera juste plus utilisé.
...
⚠️ **Si tu avais déjà essayé d'installer l'app avant et obtenu une icône
grise/vide** : supprime-la d'abord de ton écran d'accueil (appui long sur
l'icône > Supprimer), puis dans Chrome, vide le cache du site avant de
recommencer : Chrome > ⋮ > Paramètres du site > trouve ton lien
`sedonuam-ai.github.io` > Effacer et réinitialiser. Cela force Chrome à
relire le nouveau `manifest.json` avec le bon chemin d'icône.
